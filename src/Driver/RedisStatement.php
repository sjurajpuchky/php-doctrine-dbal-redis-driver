...
class RedisStatement implements IteratorAggregate, Statement
{
    /** @var string[] */
     ...
     *
     */
    public function __construct(Redis $conn, $prepareString)
    {
        $this->_conn = $conn;
        $this->_stmt = $conn->prepare($prepareString);
        if ($this->_stmt === false) {
            throw new RedisException($this->_conn->er, $this->_conn->sqlstate, $this->_conn->errno);
        }
    ...
    public function bindParam($column, &$variable, $type = ParameterType::STRING, $length = null)
    {
        if ($type === null) {
            ...
            if (! isset(self::$_paramTypeMap[$type])) {
                throw new RedisException(sprintf("Unknown type: '%s'", $type));
            }
        ...
        if ($this->_bindedValues !== null) {
            if ($params !== null) {
                if (! $this->_bindValues($params)) {
                    throw new RedisException($this->_stmt->error, $this->_stmt->errno);
                }
            } else {
                [$types, $values, $streams] = $this->separateBoundValues();
                if (! $this->_stmt->bind_param($types, ...$values)) {
                    throw new RedisException($this->_stmt->error, $this->_stmt->sqlstate, $this->_stmt->errno);
                }
    ...
    private function sendLongData($streams)
    {
        foreach ($streams as $paramNr => $stream) {
            while (! feof($stream)) {
                ...
                if ($chunk === false) {
                    throw new RedisException("Failed reading the stream resource for parameter offset ${paramNr}.");
                }
    ...
    public function fetch($fetchMode = null, $cursorOrientation = PDO::FETCH_ORI_NEXT, $cursorOffset = 0)
    {
        ...
        switch ($fetchMode) {
            ...
            default:
                throw new RedisException(sprintf("Unknown fetch type '%s'", $fetchMode));
        }
    }
